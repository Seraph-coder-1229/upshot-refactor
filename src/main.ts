...
loggingService.logInfo(`App config version loaded: ${appConfigStore.currentConfig.version}`);
...
app.config.errorHandler = (err: unknown, instance: any, info: string) => {
    ...
    const componentName = instance?.$options?.name || instance?._?.type?.name || 'UnknownComponent';
...
window.onerror = (message, source, lineno, colno, error) => {
    loggingService.logError('Global window.onerror:', {
        ...
        lineno,
        colno,
        error: error ? { name: error.name, message: error.message, stack: error.stack } : 'N/A',
    });
    ...
    if (error && (error.name === "ChunkLoadError" || error.message.includes("Failed to fetch dynamically imported module"))) {
        uiStore.setCriticalError(error, 'Failed to load a critical part of the application. Please check your network or try refreshing. If the problem persists, the application files might be corrupted or incomplete.');
    } else {
        uiStore.addNotification({ message: `An unexpected global error occurred: ${message}`, type: 'error', duration: 10000 });
    }
...
window.onunhandledrejection = (event: PromiseRejectionEvent) => {
    ...
    // } else {
       uiStore.addNotification({ message: `Unhandled promise rejection: ${errorMessage}`, type: 'error', duration: 10000 });
    // }
};
...
try {
    ...
    uiStore.setCriticalError(error, 'The application could not start due to a critical error during initialization. Please download the debug report and contact support.');
